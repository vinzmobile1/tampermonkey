// ==UserScript==
// @name         Shopee All-in-One Scraper v8.9 (Discovery Ads)
// @namespace    https://shopee.co.id/
// @version      8.9
// @description  Menambahkan deteksi "Iklan Produk Serupa" (Discovery Ads) dan menampilkan keyword pada status.
...
(function() {
    ...
    function displaySearchResults() {
        ...
        const content = `<div class="header-controls"><h1>${title}</h1><button id="downloadBtn">Download Excel</button></div><h3>Total Item: ${finalItems.length}</h3><div class="table-container"><table id="resultTable"><thead><tr>${headers.map(h => `<th${h === 'Product Name' ? ' style="min-width: 450px;"' : ''}>${h}</th>`).join('')}</tr></thead><tbody>${bodyHtml}</tbody></table></div>`;
    ...
    function extractSearchItemData(rawItem) {
        if (!rawItem || !rawItem.item_basic) return null;
        const item = rawItem.item_basic;
        let ad_info = { bid_type_str: 'Organik', ad_objective: '-', bid_price: 0, actual_cost: 0, target_keyword: '-', target_cir: 0 };
        if (rawItem.json_data) {
            const decodedAdData = decodeShopeeJsonData(rawItem.json_data);
            if (decodedAdData) {
                const bid_type_code = decodedAdData['bid-type'];
                switch (bid_type_code) {
                    case 0: ad_info.bid_type_str = "Manual"; ad_info.ad_objective = "Kata Kunci (CPC)"; break;
                    case 9: ad_info.bid_type_str = "Manual"; ad_info.ad_objective = "Kata Kunci (eCPC)"; break;
                    case 30: ad_info.bid_type_str = "Otomatis"; ad_info.ad_objective = "Iklan Toko (Target CIR)"; break;
                    case 31: ad_info.bid_type_str = "Otomatis"; ad_info.ad_objective = "Jangkauan/Trafik"; break;
                    case 37: ad_info.bid_type_str = "Otomatis"; ad_info.ad_objective = "Konversi (ROI)"; break;
                    case 41: ad_info.bid_type_str = "Otomatis"; ad_info.ad_objective = "Profit (ROAS)"; break;
                    default: ad_info.bid_type_str = `Lainnya (${bid_type_code})`; ad_info.ad_objective = 'Tidak Diketahui';
                }
                // --- PERUBAHAN ADA DI SINI ---
                if (decodedAdData['pricing-type'] === 1) {
                    ad_info.ad_objective = "Iklan Produk Serupa";
                }
                // --- AKHIR PERUBAHAN ---
                ad_info.bid_price = (decodedAdData['bid-price'] || 0) / 100000;
                ad_info.actual_cost = (decodedAdData['origin-deduction'] || 0) / 100000;
                ad_info.target_keyword = decodedAdData['bid-keyword'] || decodedAdData['keyword'] || '-';

                if (ad_info.target_keyword === "wkdaelpmissisiht") {
                    ad_info.target_keyword = "-";
                }

                if ([31,37,41].includes(bid_type_code) && decodedAdData['bid-infos']?.[0]) {
                    ad_info.target_cir = (decodedAdData['bid-infos'][0]['target_cir'] || 0) * 100;
                }
            }
        } else if (rawItem.ads_keyword) {
            ad_info.bid_type_str = 'Manual*';
            ad_info.ad_objective = 'Kata Kunci*';
            ad_info.target_keyword = rawItem.ads_keyword;
        }
        return { name: item.name||'N/A', price:item.price?item.price/100000:0, sold:item.sold||0, historical_sold:item.historical_sold||0, brand:item.brand||'-', rating_star:item.item_rating?.rating_star||0, rating_count:item.item_rating?.rating_count?.[0]||0, shop_name:item.shop_name||'N/A', shop_location:item.shop_location||'-', ctime:item.ctime?new Date(item.ctime*1000).toISOString().split('T')[0]:'N/A', stock:item.stock||0, itemid:String(item.itemid||'N/A'), shopid:String(item.shopid||'N/A'), shopee_url:createShopeeURL(item.name,item.shopid,item.itemid), image_url:item.image?`https://down-id.img.susercontent.com/file/${item.image}`:'', bid_type:ad_info.bid_type_str, ad_objective:ad_info.ad_objective, bid_price:ad_info.bid_price, actual_cost:ad_info.actual_cost, target_keyword:ad_info.target_keyword, target_cir_percent: ad_info.target_cir > 0 ? `${ad_info.target_cir.toLocaleString("id-ID",{minimumFractionDigits:1,maximumFractionDigits:1})}%` : '-' };
    }
    function extractProductDetailData(detail) { if (!detail || !detail.data || !detail.data.item) return []; const item_info = detail.data.item; const product_review_info = detail.data.product_review || {}; const shop_detailed_info = detail.data.shop_detailed || {}; const product_title = item_info.title || 'N/A'; const product_id = item_info.item_id || 'N/A'; const shop_id = item_info.shop_id || 'N/A'; const historical_sold_total = product_review_info.historical_sold || 0; let models = item_info.models || []; if (!models.length) { models = [{ name: 'Default Variation', price: item_info.price || 0, normal_stock: item_info.stock || 0, sold: historical_sold_total, model_id: 'N/A' }]; } return models.map(model => { const rating_value = item_info.item_rating?.rating_star; const final_rating = rating_value ? Number(rating_value.toFixed(1)) : 0; return { 'URL': createShopeeURL(product_title, shop_id, product_id), 'Product Name': product_title, 'Variation Name': model.name || 'N/A', 'Price': (model.price || 0) / 100000, 'Stock': model.stock || 0, 'Sold Variation': model.sold || 0, 'Total Sold': historical_sold_total, 'Rating': final_rating, 'Review': product_review_info.total_rating_count || 0, 'Brand': item_info.brand || '-', 'Model ID': String(model.model_id || 'N/A'), 'Product ID': String(product_id), 'Shop ID': String(shop_id), 'Shop Location': item_info.shop_location || '-', 'Shop Name': shop_detailed_info.name || '-' }; }); }
    ...
    function updateUI() {
        ...
        if (isShopPage) {
            ...
            if (isSearchPage) {
                ...
                statusText += `✔️ <strong style="color:#007bff;">Pencarian "${interceptedSearchData.keyword}" siap</strong> (${interceptedSearchData.items.length} item)<br>`;
            }
            if (isDetailPage) {
